...
from app.schemas import LoginIn, RegisterIn, StripeCheckoutIn, AdminTextUpdate, AdminJsonUpdate, MicrocapControlIn
from app.security import create_access_token, get_password_hash, verify_password
from app.services.access import ensure_trial, has_access
from app.services.admin_config import get_btt_preset, get_microcap_config_text, get_microcap_env, set_btt_preset, set_microcap_config_text, set_microcap_env
...
def register(payload: RegisterIn, db: Session = Depends(get_db)):
    ...
    user = User(email=payload.email.lower(), password_hash=get_password_hash(payload.password), full_name=payload.full_name, is_active=True)
    db.add(user)
    db.commit()
    db.refresh(user)
...
def public_site(db: Session = Depends(get_db)):
    ...
    return {'copy': copy, 'stripe_ready': stripe_ready(), 'microcap_live_enabled': settings.MICROCAP_LIVE_ENABLED, 'trial_hours': settings.TRIAL_HOURS}


@app.get('/api/public/microcap')
def public_microcap():
    return {'process': microcap_manager.status(), 'dashboard': read_dashboard(), 'public_mode': settings.MICROCAP_PUBLIC_MODE, 'live_available': settings.MICROCAP_LIVE_ENABLED}
...
def public_btt_latest(db: Session = Depends(get_db)):
    ...
    try:
        ...
        summary = {}
    return {'has_job': True, 'latest': {'id': job.id, 'status': job.status, 'created_at': job.created_at, 'summary': summary, 'stdout_log': job.stdout_log[-6000:], 'error_log': job.error_log[-4000:]}}
...
def user_btt_run(user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    n = db.scalar(select(func.count(BttJob.id)).where(BttJob.user_id == user.id, func.date(BttJob.created_at) == today))
...
def admin_overview(admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    ...
    return {'users': users, 'paid_users': paid, 'btt_jobs': jobs, 'microcap_process': microcap_manager.status(), 'microcap_dashboard': read_dashboard()['overview']}
...
def admin_list_btt_jobs(admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    ...
    for job in jobs:
        try:
            ...
            summary = {}
        out.append({'id': job.id, 'status': job.status, 'created_at': job.created_at, 'summary': summary, 'stdout_log': job.stdout_log[-5000:], 'error_log': job.error_log[-3000:]})
    return out
