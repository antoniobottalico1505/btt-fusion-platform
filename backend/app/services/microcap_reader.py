...
from app.services.storage import PRIVATE, user_microcap_paths

GLOBAL_DB_PATH = PRIVATE / "microcap" / "bot.db"


def _resolve_db_path(db_path: Path | None = None, user_id: int | None = None) -> Path:
    if db_path is not None:
        return db_path
    if user_id is not None:
        return user_microcap_paths(user_id)["db"]
...
def read_dashboard(db_path: Path | None = None, user_id: int | None = None) -> dict[str, Any]:
    resolved_db_path = _resolve_db_path(db_path=db_path, user_id=user_id)
